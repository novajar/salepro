...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['biller'] = $sale->user->name;
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            return view('backend.sale.create',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','lims_user_list','userId'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function checkUnitPrice($unitId){
        $units = Unit::where("id", $unit_id)
                    ->get();
    }
    ...
    public function genInvoice($id)
    {
        ...
        $kodesistem = Helper::kodeSistem('Invoices','IN');
        // dd($kodesistem);
        // var_dump($kodesistem);
        ...
        $data_inv['kodesistem'] = $kodesistem;
        $data_inv['reference_no'] = $lims_sale_data->reference_no;
        $data_inv['sales_id'] = $id;
        $data_inv['user_input'] = Auth::user()->id;
        $lims_invoice = Invoice::where('sales_id',$id)->first();
        if(empty($lims_invoice_data)){
            $create_inv = Invoice::create($data_inv);
        }
            $lims_invoice_data = Invoice::where('sales_id',$id)->first();
        $lims_delivery_data = Delivery::where('sale_id',$id)->first();
        return view('backend.sale.invoice', compact('lims_sale_data', 'lims_invoice_data','lims_delivery_data','lims_account_data','lims_sale_unit_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
